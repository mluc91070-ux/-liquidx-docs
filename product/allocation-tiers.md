---
cover: ../.gitbook/assets/gitbook-cover.png
coverY: 0
---

# Allocation Tiers

LiquidX packs are simple allocation tiers.

They help users choose how much USDT they want to allocate through the product. A pack is not a guaranteed-return product, bond, savings account, or fixed-yield plan.

## Available tiers

| Tier | Min. Deposit | Rate (approx.) | Status | Requirements |
|---|---:|---:|---|---|
| Platinum | $500 | ≈36%/mo | Open | — |
| Diamond | $1,500 | ≈40%/mo | Open | — |
| Captain | $5,000 | ≈44%/mo | Locked | 5 qualified invites |
| Ambassador | $15,000 | ≈47%/mo | Locked | 15 qualified invites |
| VIP | $50,000 | ≈50%/mo | Locked | Contact desk |

Rates are **variable**, set from the previous month's average. They are not guaranteed returns. Capital is at risk.

**Locked tiers** (Captain, Ambassador, VIP) require a minimum number of qualified invites to unlock. 1 qualified invite = $1,000 deposited by an invited user. Whales may request a manual upgrade.

### Illustrative earnings

The table below shows what the app may project based on current rates. These are illustrative only.

| Tier | On deposit | Daily ≈ | Monthly ≈ | With auto-compound/mo |
|---|---:|---:|---:|---:|
| Platinum | $500 | $6.00 | $180.00 | $215.13 |
| Diamond | $1,500 | $20.00 | $600.00 | $731.83 |
| Captain | $5,000 | $73.33 | $2,200.00 | $2,738.77 |
| Ambassador | $15,000 | $235.00 | $7,050.00 | $8,912.62 |
| VIP | $50,000 | $833.33 | $25,000.00 | $32,097.05 |

Auto-compound projections assume yield is reinvested daily. Actual results may differ significantly.

The tier amount represents the user's allocation level. Product rules, availability, lock periods, referral treatment, and withdrawal conditions may vary by tier and should be reviewed inside the official app.

## Tier perks

Each tier includes access features beyond the base allocation:

* **Platinum** — Early access to new features.
* **Diamond** — Weekly desk notes and access to the Diamond lounge.
* **Captain** — Lead a local community group and receive a co-marketing kit.
* **Ambassador** — Host official LiquidX events in your country.
* **VIP** — Direct desk line, custom allocation, and top-tier events.

## Why users choose a tier

Users choose a tier because it gives them access to a simple allocation level inside LiquidX.

A pack allows a user to participate in liquidity activity, track dashboard status, and access referral rewards without managing complex market strategies themselves.

The user does not need to manage DeFi pools, trading bots, market-making systems, OTC relationships, or private execution routes manually.

## Why packs exist

Packs make liquidity allocation easier for retail users.

Instead of asking users to build their own strategy, manage exchange accounts, or understand every liquidity route, LiquidX groups access into clear tiers that can be tracked from the dashboard.

Packs may help LiquidX:

* Organize user allocations.
* Group capital into Community Vaults.
* Route capital into liquidity pools or vaults.
* Apply product rules consistently.
* Track performance and activity.
* Calculate rewards and referrals.
* Manage withdrawal cycles.

## What happens after choosing a pack

After a deposit is confirmed and a pack is selected, capital may enter internal liquidity routes, managed pools, or Community Vaults.

Those structures may support activity such as OTC liquidity demand, crypto market liquidity, stablecoin flows, market-making operations, commodities-related liquidity needs, private execution routes, or internal liquidity balancing.

If eligible activity generates fees or performance, the dashboard may update accordingly.

## How packs connect to rewards

Rewards may be calculated from eligible liquidity fees or vault performance.

The calculation may depend on:

* Pack selected.
* Amount allocated.
* Vault or pool activity.
* Liquidity demand.
* Market conditions.
* Fees and costs.
* Product rules.
* Risk controls.
* Withdrawal status.

Rewards are variable and not guaranteed.

## What packs do not mean

A pack does not mean:

* Guaranteed monthly profit.
* Fixed yield.
* Capital protection.
* Instant withdrawal in every condition.
* Risk-free income.
* Automatic referral earnings.

Packs are allocation tiers. Performance remains variable and capital remains at risk.

## Choosing a tier

Users should choose a tier based on risk tolerance, product understanding, and money they can afford to place at risk.

Starting with Platinum can help a new user learn the app, dashboard, withdrawal process, reward logic, and risk model before moving to higher tiers.

---

> **Risk notice:** LiquidX does not guarantee returns. Performance is variable. Capital is at risk. Rewards may be lower than projected, delayed, or negative depending on liquidity demand, market activity, execution quality, counterparties, vault performance, and product rules. Nothing in this documentation is financial advice. Use only the official LiquidX bot: **[@LiquidX_official_bot](https://t.me/LiquidX_official_bot)**.
